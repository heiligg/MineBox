...
def start_service() -> CommandResult:
    """Low-level systemd/dev start (no formal state machine)."""
    if is_running():
        return CommandResult(True, "Minecraft is already running.")
...
def start() -> CommandResult:
    """Start via formal lifecycle manager (Checkpoint 3)."""
    from services.lifecycle import get_lifecycle

    return get_lifecycle().start()
...
def stop_service() -> CommandResult:
    """Low-level systemd/dev stop (no formal state machine)."""
    if not is_running():
        return CommandResult(True, "Minecraft is already offline.")
...
def stop() -> CommandResult:
    """Stop via formal lifecycle manager (Checkpoint 3)."""
    from services.lifecycle import get_lifecycle

    return get_lifecycle().stop()


def restart_service() -> CommandResult:
    """Low-level systemd/dev restart (no formal state machine)."""
    if is_running():
        save_world()
    if _dev_mode():
...
def restart() -> CommandResult:
    """Restart via formal lifecycle manager (Checkpoint 3)."""
    from services.lifecycle import get_lifecycle

    return get_lifecycle().restart()
