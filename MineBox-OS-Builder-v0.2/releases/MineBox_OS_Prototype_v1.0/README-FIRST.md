...
SSH is **enabled** in `config/minebox-pi5.conf` (`ENABLE_SSH=1`) for appliance bring-up. Change the password (and disable SSH if undesired) before distributing an image. Local console recovery remains on tty2; curses UI owns tty1.
