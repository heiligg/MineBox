...
required_nonempty=(
  ...
  "app/menu.py"
  "app/api/server.py"
  "app/services/minebox-api.service"
  "app/services/minebox-network.service"
  "app/services/minebox-update.service"
  "app/scripts/minebox_update_apply.py"
  ...
  "pi-gen/stage-minebox/01-system-config/files/minecraft.service"
...
# Fail early if hotspot settings drift away from the supported SoftAP profile.
#
# channel=11: preferred 2.4 GHz SoftAP channel for MineBox (often quieter than
# channel 1/6 in dense environments). Validated in production hotspot configs.
#
# no-resolv: required so dnsmasq does NOT inherit host resolvers. SoftAP clients
# must use the Pi (192.168.4.1) as DNS so minebox.setup / captive names resolve
# locally even without uplink. Upstream names are forwarded via explicit server=.
HOTSPOT_HOSTAPD="$ROOT_DIR/pi-gen/stage-minebox/02-dedicated-hotspot/files/hostapd.conf"
HOTSPOT_DNSMASQ="$ROOT_DIR/pi-gen/stage-minebox/02-dedicated-hotspot/files/dnsmasq-minebox.conf"

grep -Fqx 'ssid=MineBox-Setup' "$HOTSPOT_HOSTAPD"
grep -Fqx 'hw_mode=g' "$HOTSPOT_HOSTAPD"
grep -Fqx 'channel=11' "$HOTSPOT_HOSTAPD"
grep -Fqx 'wpa=2' "$HOTSPOT_HOSTAPD"
grep -Fqx 'rsn_pairwise=CCMP' "$HOTSPOT_HOSTAPD"
...
grep -Fqx 'bind-dynamic' "$HOTSPOT_DNSMASQ"
grep -Fqx 'no-resolv' "$HOTSPOT_DNSMASQ"
grep -Fq 'dhcp-option=option:dns-server,192.168.4.1' "$HOTSPOT_DNSMASQ"
...
# Example configuration files required for Checkpoint 2+.
[ -s "$ROOT_DIR/config/minebox.example.toml" ] || {
  echo "Missing config/minebox.example.toml"
  exit 1
}
[ -s "$ROOT_DIR/config/hardware.example.toml" ] || {
  echo "Missing config/hardware.example.toml"
  exit 1
}
...
# Checkpoint 7: appliance packages + SoftAP renderer + canonical sudoers.
grep -Fqx 'chromium' "$ROOT_DIR/pi-gen/stage-minebox/00-install-minebox/00-packages"
grep -Fqx 'xinit' "$ROOT_DIR/pi-gen/stage-minebox/00-install-minebox/00-packages"
grep -Fq 'services/sudoers/minebox' "$ROOT_DIR/pi-gen/stage-minebox/00-install-minebox/00-run-chroot.sh"
[ -s "$ROOT_DIR/app/services/sudoers/minebox" ] || {
  echo "Missing app/services/sudoers/minebox"
  exit 1
}
[ -s "$ROOT_DIR/app/scripts/minebox_render_hotspot_configs.py" ] || {
  echo "Missing minebox_render_hotspot_configs.py"
  exit 1
}
[ -s "$ROOT_DIR/app/scripts/minebox_validate_install.py" ] || {
  echo "Missing minebox_validate_install.py"
  exit 1
}
grep -Fq 'minebox_render_hotspot_configs.py' \
  "$ROOT_DIR/pi-gen/stage-minebox/01-system-config/files/minebox-firstboot"
! grep -Fq 'network-online.target' "$ROOT_DIR/app/services/minebox-api.service"
! grep -Fq 'network-online.target' "$ROOT_DIR/app/services/minebox-captive.service"
