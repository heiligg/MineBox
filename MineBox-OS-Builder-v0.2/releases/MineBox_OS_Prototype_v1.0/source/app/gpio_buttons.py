...
Default v1 actions (configurable via hardware.toml):
  Short Left  → KEY_UP     (nav previous)
  Short Right → KEY_DOWN   (nav next)
...
# Fallback only if config cannot load — still marked provisional in docs.
...
class ButtonController:
    ...
    def start(self) -> bool:
        ...
        self._thread.start()
        LOGGER.info(
            "Buttons active from config (UNVERIFIED_AGAINST_PCB): "
            "short L/R=nav, hold L=back, hold R=confirm (BCM %s/%s)",
            self.left_gpio,
            self.right_gpio,
        )
        return True
    ...
    def _handle(self, pin: _DebouncedPin, now: float, on_short: Callable[[], None], on_hold: Callable[[], None]) -> None:
...
# Back-compat names for scripts/docs that imported module constants.
