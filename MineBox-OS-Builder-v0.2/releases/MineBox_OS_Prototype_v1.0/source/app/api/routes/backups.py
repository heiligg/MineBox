...
def delete_backup(
    ...
    confirmed = bool(confirm or (body.confirm if body is not None else False))
    if not confirmed:
        raise HTTPException(
            status_code=400,
            detail="Backup deletion requires confirm=true.",
        )
    try:
...
def restore_backup(
    ...
    confirmed = bool(confirm or (body.confirm if body is not None else False))
    try:
        restored = backups.restore_backup(filename, confirm=confirmed)

    except backups.BackupError as exc:
        message = str(exc)

        status = _backup_error_status(message)
        if "confirm=true" in message.lower():
            status = 400
