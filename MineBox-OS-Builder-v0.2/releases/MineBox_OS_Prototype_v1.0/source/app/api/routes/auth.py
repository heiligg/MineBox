...
from core.csrf import ensure_csrf_token, rotate_csrf_token
from core.rate_limit import check_rate_limit, clear, client_key, record_failure
from services import auth
import logging

LOGGER = logging.getLogger("minebox.auth.routes")
...
async def complete_setup(
    ...
    key = client_key(request, "auth-setup")
    allowed, retry = check_rate_limit(
        key, max_attempts=8, window_s=600, cooldown_s=180
    )
    if not allowed:
        return JSONResponse(
            {
    ...
    clear(key)
    LOGGER.info("auth_setup_success user=%s", username.strip())
...
async def login(
    ...
    key = client_key(request, "auth-login")
    allowed, retry = check_rate_limit(
        key, max_attempts=8, window_s=300, cooldown_s=120
    )
    if not allowed:
        ...
        return JSONResponse(
            {
                "ok": False,
                "detail": f"Too many login attempts. Try again in {int(retry)}s.",
            },
            status_code=429,
        )
    ...
    # Session fixation protection: clear then re-issue.
    request.session.clear()
    request.session["authenticated"] = True
    request.session["username"] = username.strip()
    rotate_csrf_token(request.session)
    clear(key)
    LOGGER.info("auth_login_success user=%s", username.strip())
...
def authentication_status(
    ...
    return {
        ...
        "password_algo": auth.password_algo() if auth.is_configured() else None,
        "first_boot": first_boot.status(),
        "csrf_token": ensure_csrf_token(request.session),
    }
...
async def change_password(request: Request):
    ...
    key = client_key(request, "change-password")
    allowed, retry = check_rate_limit(
        key, max_attempts=6, window_s=600, cooldown_s=180
    )
    if not allowed:
        return JSONResponse(
            {
                "ok": False,
                "detail": f"Too many password changes. Retry in {int(retry)}s.",
            },
            status_code=429,
        )
    ...
    mark_credential_rotated("dashboard_admin")
    clear(key)
