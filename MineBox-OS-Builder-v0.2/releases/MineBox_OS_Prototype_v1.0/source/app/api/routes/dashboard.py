...
SERVER_JAR = servers.MINECRAFT_ROOT / "server.jar"
SERVER_PROPERTIES = servers.MINECRAFT_ROOT / "server.properties"
...
@router.get("/first-boot", include_in_schema=False)
def first_boot_page(request: Request):
    return templates.TemplateResponse(
        request=request,
        name="first_boot.html",
        context={},
    )
...
def dashboard(request: Request):
    ...
    if not first_boot.is_appliance_setup_complete():
        return RedirectResponse(url="/first-boot", status_code=307)
