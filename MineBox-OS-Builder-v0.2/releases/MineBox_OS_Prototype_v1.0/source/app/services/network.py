...
def hotspot_interface() -> str:
    """Resolved SoftAP interface (identity-based roles; not name-only)."""
    try:
        from networking.roles import resolve_roles

        assignment = resolve_roles(persist=False)
...
def __getattr__(name: str):
    # Backward-compatible dynamic alias — never a permanent wlan0 assumption.
...
def client_wifi_interface() -> str | None:
    ...
    setup-hotspot radio. Falls back to any NM Wi-Fi device.
    """

    ap = hotspot_interface()
    try:
        from networking.roles import resolve_roles

        role_wifi = resolve_roles(persist=False).wifi_uplink
        if role_wifi:
            return role_wifi
    except Exception:
        pass
...
def hotspot_radio_in_use() -> bool:
    """True when the AP radio is busy with the setup hotspot."""
