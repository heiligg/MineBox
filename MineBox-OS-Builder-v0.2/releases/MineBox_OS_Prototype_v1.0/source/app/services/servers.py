...
# Prototype v1: Paper + Vanilla are official; others preserved as experimental.
PROVIDER_STATUS = {
    "vanilla": "SUPPORTED",
    "paper": "SUPPORTED",
    "fabric": "EXPERIMENTAL",
...
def provider_capabilities() -> list[dict]:
    try:
        from minecraft.providers import provider_capabilities as _caps

        return _caps()
    except Exception:
        rows = []
        for loader in SUPPORTED_LOADERS:
            status = PROVIDER_STATUS.get(loader, "EXPERIMENTAL")
            rows.append(
                {
                    "id": loader,
                    "name": loader,
