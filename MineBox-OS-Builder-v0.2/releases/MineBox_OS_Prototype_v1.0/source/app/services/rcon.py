...
from config import RCON_HOST, RCON_PORT, get_rcon_password
...
def ensure_properties(server_dir: Path | None = None) -> dict[str, str]:
    ...
    try:
        ...
        password = ""
    if not password:
        from core.secrets_store import get_or_create_server_rcon_password

        password = get_or_create_server_rcon_password(server_dir)
...
def send(command: str, timeout: float = 4.0, *, enforce_allowlist: bool = False) -> CommandResult:
