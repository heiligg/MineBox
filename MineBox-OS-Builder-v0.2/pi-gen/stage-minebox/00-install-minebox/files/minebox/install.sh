...
# Keep in sync with app/services/sudoers/minebox (args need explicit * entries).
if [ -f "$TARGET_DIR/services/sudoers/minebox" ]; then
  install -m 0440 "$TARGET_DIR/services/sudoers/minebox" "$SUDOERS_FILE"
else
  cat > "$SUDOERS_FILE" <<'SUDOERS'
minebox ALL=(root) NOPASSWD: /usr/bin/systemctl start minecraft.service, /usr/bin/systemctl stop minecraft.service, /usr/bin/systemctl restart minecraft.service, /usr/bin/systemctl start minebox-update.service, /usr/bin/systemctl stop hostapd.service, /usr/bin/systemctl start hostapd.service, /usr/bin/systemctl stop dnsmasq.service, /usr/bin/systemctl start dnsmasq.service, /usr/bin/systemctl enable avahi-daemon.service, /usr/bin/systemctl start avahi-daemon.service, /usr/bin/systemctl try-reload-or-restart avahi-daemon.service, /usr/bin/python3 /opt/minebox/scripts/minebox_fix_minecraft_perms.py, /usr/local/sbin/minebox-fix-minecraft-perms, /usr/bin/python3 /opt/minebox/scripts/minebox_install_avahi.py, /usr/local/sbin/minebox-install-avahi, /usr/bin/python3 /opt/minebox/scripts/minebox_ensure_java.py, /usr/bin/python3 /opt/minebox/scripts/minebox_ensure_java.py *, /usr/local/sbin/minebox-ensure-java, /usr/local/sbin/minebox-ensure-java *, /usr/bin/python3 /opt/minebox/scripts/minebox_ensure_tls.py, /usr/bin/python3 /opt/minebox/scripts/minebox_ensure_tls.py *, /usr/local/sbin/minebox-ensure-tls, /usr/local/sbin/minebox-ensure-tls *, /usr/bin/systemctl restart minebox-api.service, /usr/bin/python3 /opt/minebox/scripts/minebox_fan_test.py *, /usr/local/sbin/minebox-fan-test *, /usr/bin/journalctl -u minecraft.service *, /usr/bin/systemctl poweroff, /usr/bin/systemctl reboot, /usr/bin/python3 /opt/minebox/scripts/minebox_set_os_password.py minebox
SUDOERS
...
if [ -f "$TARGET_DIR/scripts/minebox_ensure_java.py" ]; then
  install -m 0755 "$TARGET_DIR/scripts/minebox_ensure_java.py" /usr/local/sbin/minebox-ensure-java
fi
