...
def _network_devices() -> list[dict[str, str]]:
    """Return non-loopback devices known to NetworkManager."""
    result = _nmcli([
        "--terse", "--fields", "DEVICE,TYPE,STATE,CONNECTION",
        "device", "status",
    ])
    if not result.ok:
        return []
    ...
    for raw_line in result.stdout.splitlines():
        ...
        while len(fields) < 4:
            fields.append("")
        device, device_type, state, connection = fields[:4]
        if not device or device == "lo" or device_type in {"loopback", "bridge"}:
            continue
        devices.append({
            "device": device,
            "type": device_type,
...
def _default_route_interface() -> str | None:
    """Return the interface carrying the system default route."""
    result = run(["ip", "route", "show", "default"], timeout=10)
    if not result.ok:
        return None
...
def status() -> dict[str, Any]:
    ...
    selected = next(
        (d for d in devices if d["device"] == default_interface),
        None,
    )
    if selected is None:
        selected = next((d for d in devices if d["state"] == "connected"), None)

    interface = selected["device"] if selected else default_interface
    addresses = _device_ipv4(interface) if interface and available else []
    gateway = _device_gateway(interface) if interface and available else None
    dns = _device_dns(interface) if interface and available else []

    connection_type = None
    if selected:
        if selected["type"] in {"ethernet", "802-3-ethernet"}:
            connection_type = "ethernet"
        elif selected["type"] == "wifi":
            connection_type = "wifi"
        else:
            connection_type = selected["type"] or None

    connected = bool(interface and (addresses or (selected and selected["state"] == "connected")))
    hotspot_active = hotspot_is_active() if available else False
    if hotspot_active and connection_type == "wifi":
        connection_type = "hotspot"

    response: dict[str, Any] = {
        "networkmanager_available": available,
        "wifi_available": wifi is not None,
        "interface": interface,
        "hostname": _hostname(),
        ...
        "connection_name": (
            selected["connection"]
            if selected and selected["connection"] not in {"", "--"}
            else None
        ),
        "ssid": None,
        "signal": None,
        ...
        "ip_address": addresses[0].split("/", 1)[0] if addresses else None,
        "gateway": gateway,
        "dns": dns,
    ...
    if selected and selected["type"] == "wifi":
        active_wifi = _active_wifi_details(selected["device"])
